    ...
    <div className="page-shell">
      <div className="page-content">
        ...
        <main className="space-y-8">
          <section className="app-panel-subtle px-5 py-6 md:px-8 md:py-8">
          ...
          <section>
            <h2 className="section-title mb-5">Recent Positions</h2>
            ...
            {isLoading ? (
              <div className="grid gap-4 lg:grid-cols-3">
                {Array.from({ length: 3 }, (_, i) => (
                  <div
                    key={i}
                    className="app-panel-subtle animate-pulse h-40"
                  />
                ))}
              </div>
            ) : isError ? (
              <div className="app-panel-subtle px-5 py-6">
              ...
              <div className="grid gap-4 lg:grid-cols-3">
                {summary.recentMarkets.map((market) => (
                  <Link
                    key={market.marketId}
                    className="app-panel-subtle px-5 py-5 text-[inherit] no-underline transition-transform duration-200 hover:-translate-y-1"
                    to={getMarketPath(market.marketId)}
                  >
                    <p className="eyebrow mb-2">
                      {market.marketStatus === "OPEN" ? "Open" : "Resolved"}
                    </p>
                    <h3 className="type-body-lg font-semibold leading-snug mb-4">
                      {market.marketName}
                    </h3>
              ...
              <div className="app-panel-subtle px-5 py-6">
                ...
                <Link className="action-secondary mt-4 inline-flex no-underline" to="/">
                  Browse markets
                </Link>
              </div>
            )}
